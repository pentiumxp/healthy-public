...
test("MCP wrapper lists mcp_health callable from workspace-local config", () => {
  ...
  for (const name of [
    ...
    "mcp_health_apple_health_bulk_sync", "mcp_health_apple_health_sync_state_get", "mcp_health_apple_health_guardian_status_get", "mcp_health_apple_health_incremental_sync",
...
test("MCP wrapper can write and read workspace-local health data", async () => {
  ...
  try {
    ...
    const guardianStatus = await mcpCall(workspace, "mcp_health_apple_health_guardian_status_get", {});
    assert.equal(guardianStatus.mode, "guardian_freshness");
    assert.equal(guardianStatus.domains.daily_summaries.last_sample_at, "2026-06-15");
    assert.equal(guardianStatus.instructions.write_endpoint, "/api/v1/apple-health/incremental-sync");
