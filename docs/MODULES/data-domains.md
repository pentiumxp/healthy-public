...
健康守护模式不新增 raw health payload 表。Healthy 在
`apple_health_guardian_status` 中只保存每个 workspace/user 的 bounded 客户端
状态：enabled/disabled、最近成功上传、客户端上报的最近失败、失败码、短失败摘要、
client sync id、source 和 range。AI 或 UI 通过
`GET /api/v1/apple-health/guardian-status` 或
`mcp_health_apple_health_guardian_status_get` 查询 freshness；响应按
daily/workout/sleep/ECG/vitals/body measurements 给出最新样本时间、最新上传时间和
stale/missing 状态。输出只能用于说明 Apple Health 数据是否足够新，不能暗示 iOS
后台可以保证连续实时监控。
